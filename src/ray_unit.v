...
module brick_texture (
    ...
    input  wire [4:0]      ray_depth_in, // Assuming MAX_RAY_DEPTH=31, so 5 bits (4:0)
    output wire [4:0]      color_out
);
    ...
    assign color_out = fp_gt(fp_min(ex, ey), `FP_ZERO) ? ray_depth_in - 5'd2 : ray_depth_in;
endmodule
...
module ray_unit #(
    ...
    wire [4:0] texture_out;
    wire [4:0] current_color;
    ...
    assign hit  = (sdf_dist < (`FP_HUNDREDTH >> 1));
    assign miss = (sdf_dist > `FP_FIVE) || (ray_depth == MAX_RAY_DEPTH);
    assign dither_correction = ((current_color >> 1) != 4'd15) && toggle_dither_in && current_color[0] && (hcount[0] ^ vcount[0]);
